...
in_pd = pd.read_csv("train_v2.csv", dtype={'fullVisitorId': 'str'})
k = 0
count = 0
for i in range(int(in_pd.shape[0]/16), in_pd.shape[0] + int(in_pd.shape[0]/16) , int(in_pd.shape[0]/16)):
    x = in_pd[k:i]
    x.to_csv("train_part_"+str(count)+".csv", index = False)
    k = i
    count = count + 1
...
for fileNumber in range(16):
    read_csv_path = "train_part_"+str(fileNumber)+".csv"
    ...
    save_csv_path = "flatten_train_part_"+str(fileNumber)+".csv"
